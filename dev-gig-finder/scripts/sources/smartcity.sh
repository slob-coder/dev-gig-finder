...
SOURCE_NAME="智城外包网"
# 尝试多个可能的域名
CHECK_URLS=("https://www.taskcity.com/" "https://www.smartcity.com/")
...
fetch_smartcity() {
  ...
  if [ -z "$base_url" ] || [ -z "$html" ]; then
    log_warn "$SOURCE_NAME 所有域名均不可达，跳过"
    echo "[]"
    return 0
  fi
  ...
  while IFS= read -r line; do
    ...
    count=$((count + 1))
  done < <(echo "$html" | grep -iE '(项目|需求|外包|开发)' | grep -i 'href' | head -"$((limit * 2))")
